...
/*`
191127-2023 interpolation trials:
191127-2023	for (i = 0, l = ndl / tb; i < l; )
				for (j = 0; j < tc; i++, j++) {
					//rnd16[i] = rwd16[(int)floor(((double)i / ts) * ival1 + .5f)];	//`first working floating point tech
					rnd16[i] = rwd16[(long long)i * ival1 / ts];
				}
	-2046 first working 16-bit nearest-neighbor resample
			for (i = 0, l = ndl / wbps; i < l; i++)
				for (j = 0; j < tc; j++) {
					rnd16[i*tc + j] = rwd16[(((long long)i * ts) / ival1) * tc + j];
				}
191128-0125 all resampling stuff works :) also impld amp real quick in like ~5min before linear resamp
	-0226	all ops impld! copying just impld in like ~10 min. Will test tomorrow! orig quick impl:
	case 50:				//copy
		nd = kmalloc(ndl);	//already a bug ;), ndl not assigned till later
		memcpy(nd, wd, ndl);
		wdle = wdl;
		wde = wd;
	case 51:				//copyext
		if (!nd) nd = wd;
		ndl = wdl;
		if (ssp < 0) memset(nd + sdp, 0, -ssp);
		if (ssp + slen > wdle) memset(nd + sdp + (wdle - ssp), 0, wdle - (ssp + slen));
		memcpy(nd + sdp, wde + ssp, slen);
		break;
191128-1541	v1.0.0 all done!!! TODOs for future: write only modified region, support more formats.
191129-0355	added early terminations (ret 1) for args that result in no changes
